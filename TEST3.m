function anonymiseDicomsManuallyATWM1()
...
if ~exist(folderDefinition.anonymizedDataArchiveHighResAnatomySubjectAnon, 'dir')
    mkdir(folderDefinition.anonymizedDataArchiveHighResAnatomySubjectAnon)
end

%%% Anonymize DICOM files and copy them to archive folder for anonymized
%%% data
nrOfDicomFiles = numel(aStrPathOriginalDicomFilesVmrHighRes);
for cf = 1:nrOfDicomFiles
    strNew = strcat(parametersDicomFileAnonymisation.strAnonymized, '_', aStrOriginalDicomFilesVmrHighRes{cf});
    strAnonDicomFile = strrep(aStrOriginalDicomFilesVmrHighRes{cf}, aStrOriginalDicomFilesVmrHighRes{cf}, strNew);
    strPathOriginalDicomFilesSubjectAnonDicomFile{cf}        = fullfile(folderDefinition.anonymizedDataArchiveHighResAnatomySubjectAnon, strAnonDicomFile);
...
strPathLocalZipFileAnonymisedHighResAnatomy = fullfile(folderDefinition.anonymizedDataArchiveHighResAnatomySubject, strZipFileAnonymisedHighResAnatomy);
%%{
try
    zip(strPathLocalZipFileAnonymisedHighResAnatomy, folderDefinition.anonymizedDataArchiveHighResAnatomySubjectAnon);
    success = 1;
catch
    success = 0;
...
if success
    ...
    try
        rmdir(folderDefinition.anonymizedDataArchiveHighResAnatomySubjectAnon, 's');
        success = true;
    catch
        success = false;
