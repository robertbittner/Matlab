function structMissingArchiveData = TEST4()
...
folderDefinition = addServerFolderDefinitionsATWM1(folderDefinition);

aSubject = processSubjectArrayATWM1_IMAGING;

aStrGroups = fieldnames(aSubject.ATWM1_IMAGING.Groups);

nrOfGroups = numel(aStrGroups) - 1;


%return
for cg = 1:nrOfGroups
    nrOfSubjectsGroup = aSubject.ATWM1_IMAGING.nSubjects.(matlab.lang.makeValidName(aStrGroups{cg}));
    %if nrOfSubjectsGroup > 0
        strGroup = aStrGroups{cg};
        strArchiveFolderGroup = strcat(folderDefinition.archiveDICOMfiles, strGroup, '\');
        aStrSubjectsGroup = aSubject.ATWM1_IMAGING.Groups.(matlab.lang.makeValidName(aStrGroups{cg}));
        counterMissingArchiveData(cg) = 0;
        for cs = 1:nrOfSubjectsGroup
            ...
            strPathZipFileSingleSubjectData = fullfile(strArchiveFolderGroup, strZipFileArchiveDicomFilesSubject);
            
            structSubjectArchiveFolders = defineSubjectArchiveFoldersATWM1(folderDefinition);
            %continue
            %structSubjectArchiveFolders.strFolderLocalArchiveDicomFilesSubject 
            ...
            if ~exist(strPathZipFileSingleSubjectData, 'file')
                counterMissingArchiveData(cg) = counterMissingArchiveData(cg) + 1;
                structMissingArchiveData.aStrSubjects{cg, counterMissingArchiveData(cg)} = strSubject;
                structMissingArchiveData.aStrZipFileArchiveDicomFiles{cg, counterMissingArchiveData(cg)} = strZipFileArchiveDicomFilesSubject;
                structMissingArchiveData.aStrPathZipFileArchiveDicomFiles{cg, counterMissingArchiveData(cg)} = strPathZipFileSingleSubjectData;
                parametersMriSession = analyzeParametersMriScanFileATWM1;
                if isempty(parametersMriSession)
                    structMissingArchiveData.bParametersMriSessionMissing(cg, counterMissingArchiveData(cg)) = true;
                else
                    structMissingArchiveData.bParametersMriSessionMissing(cg, counterMissingArchiveData(cg)) = false;
                end
                %%% Search for missing presentation logfiles
                structMissingArchiveData.bPresentationLogfilesMissing(cg, counterMissingArchiveData(cg)) = false;
                for clf = 1:nLogfiles
                    if ~exist(strucPathPresentationLogfiles.aStrPathServerPresentationLogfiles{clf}, 'file')
                        %test = strucPathPresentationLogfiles.aStrPathServerPresentationLogfiles{clf}
                        structMissingArchiveData.bPresentationLogfilesMissing(cg, counterMissingArchiveData(cg)) = true
                    end
                end
            end
        end
...
for cg = 1:nrOfGroups
    strTextFile = sprintf('Missing_Archive_Data_%s.txt', aStrGroups{cg});
    strPathTextFile = fullfile(folderDefinition.archiveDICOMfiles, strTextFile);
    fid = fopen(strPathTextFile, 'wt');
    for cs = 1:counterMissingArchiveData(cg)
        fprintf(fid, '%s\t%i\t%i\n', structMissingArchiveData.aStrSubjects{cg, cs}, structMissingArchiveData.bParametersMriSessionMissing(cg, cs), structMissingArchiveData.bPresentationLogfilesMissing);
    end
    fclose(fid);
