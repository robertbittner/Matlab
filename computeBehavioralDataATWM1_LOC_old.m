...
function tempTrialData = addTrialInformationATWM1_LOC(parametersParadigm, tempTrialData, strBehavioralDataFile);

for ct = 1:parametersParadigm.nTrialsPerRun
    trialInfo = tempTrialData.strTrialInfo{ct};
...
function tempTrialData = determineTrialAccuracyATWM1_LOC(parametersParadigm, tempTrialData);

for ct = 1:parametersParadigm.nTrialsPerRun
    
    recordedResponse = tempTrialData.response(ct);
    
    %%% Determine which response is appropriate for a given trial
    for cco = 1:length(parametersParadigm.aConditions)
        if strcmp(tempTrialData.condition{ct}, parametersParadigm.aConditions{cco}) == 1
            expectedResponse = parametersParadigm.vValidResponses(cco);
        end
    end
...
function tempTrialData = calculateTrialReactionTimeATWM1_LOC(parametersParadigm, tempTrialData);
% Calculate the reaction time for each target trial and transform it to
% millisecond time scale

for ct = 1:parametersParadigm.nTrialsPerRun
    if strcmp(tempTrialData.condition{ct}, parametersParadigm.aConditions{1}) == 1
        reactionTime = tempTrialData.responseOnset(ct) - tempTrialData.stimulusOnset(ct);
    else
        reactionTime = 0;
    end
...
function saveBehavioralDataATWM1_LOC(trialData, strBehavioralDataFile, strPathBehavioralDataFile);
...
aStrFieldNames = fieldnames(trialData);
...
for cfn = 1:length(aStrFieldNames)
    aTrialData.(genvarname(aStrFieldNames{cfn}))= trialData.(genvarname(aStrFieldNames{cfn}));
end
    %end
%end
...
function subjectPerformanceData = calculateSubjecPerformanceDataATWM1_LOC(parametersParadigm, parametersAnalysisBehavioralData, strBehavioralDataFile, strPathBehavioralDataFile);
...
%test = mean(aTrialData.iTrialAccuracy)

% Determine target and standard trials
vTargetTrials = strcmp(aTrialData.condition, parametersParadigm.iTargetTrial);
vStandardTrials = strcmp(aTrialData.condition, parametersParadigm.iStandardTrial);

nTargetTrials = sum(vTargetTrials)

% Compute overall accuracy
subjectPerformanceData.overallAccuracy = mean(aTrialData.iTrialAccuracy);

% Compute accuracy for target trials
subjectPerformanceData.targetTrialAccuracy = mean(aTrialData.iTrialAccuracy(vTargetTrials));

% Compute accuracy for standard trials
subjectPerformanceData.standardTrialAccuracy = mean(aTrialData.iTrialAccuracy(vStandardTrials));

% Compute reaction time for target trials
subjectPerformanceData.targetTrialReactionTime = mean(aTrialData.reactionTime(vTargetTrials));
...
function [strSpssBehavioralDataFile, strExcelBehavioralDataFile, successTextFileCreation, successExcelFileCreation] = writeComputedBehavioralDataToFileATWM1_LOC(parametersStudy, parametersParadigm, parametersAnalysisBehavioralData, folderDefinition, aSubject, nGroups, nSubjects, subjectPerformanceData);
...
strSpssBehavioralDataFile = sprintf('%s_%s_%i_%s_%i_subj_BHD_SPSS.txt', iStudy, 'LOC', nGroups, strGroup, sum(nSubjects));
...
strExcelBehavioralDataFile = sprintf('%s_%s_%i_%s_%i_subj_BHD_SPSS.xlsx', iStudy, 'LOC', nGroups, strGroup, sum(nSubjects));
