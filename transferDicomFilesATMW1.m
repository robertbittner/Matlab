...
hFunction = str2func(sprintf('defineParametersForFileTransfer%s', iStudy));
[bCreateProjectFiles, bAbort] = feval(hFunction, strStudyType);
if bAbort == true
    return
end
...
%%% Remove
iSubject = 'PAT01';
iGroup = parametersGroups.aStrShortGroups{1};
iGroupLong = parametersGroups.aStrLongGroups{1};
iSession = 1;
...
folderDefinition.strFolderTransferFromScanner = strcat(folderDefinition.singleSubjectData, iGroup, '\', iSubject, '\');
%}
...
while bSingleSubjectDataComplete == false
    ...
    %%% ParametersMriScan
    %{
    strParametersMriScanFile = sprintf('%s%sParametersMriScan.m', iSubject, iStudy);
...
[iSubject, iGroup, iGroupLong, bAbort] = feval(hFunction, strStudyType);
if bAbort == true
    return
end
...
strButton1 = sprintf('%sFile transfer & Project file creation %s', parametersDialog.strEmpty, parametersDialog.strEmpty);
strButton2 = sprintf('%sFile transfer only%s', parametersDialog.strEmpty, parametersDialog.strEmpty);
strButton3 = sprintf('%sAbort%s', parametersDialog.strEmpty, parametersDialog.strEmpty);
choice = questdlg(strQuestion, strTitle, strButton1, strButton2, strButton3, strButton1);
if ~isempty(choice)
    switch choice
        case strButton1
...
strZipFileSingleSubjectData = sprintf('%s_%s.zip', iSubject, iStudy);
pathZipFileSingleSubjectData = strcat(folderDefinition.archiveZip, strZipFileSingleSubjectData);
strMessage = sprintf('Storing files for %s in file %s', iSubject, pathZipFileSingleSubjectData);
disp(strMessage);
