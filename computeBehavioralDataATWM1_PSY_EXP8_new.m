...
%aSubject = feval(str2func(strcat('aSubject', iStudy,)));   test 21/01
aSubject = feval(str2func(strcat('aSubject', iStudy,'_PSY')));
aSubject = aSubjectATWM1_PSY

aStrGroups = fieldnames(aSubject.ATWM1_PSY.Groups)
nGroups=length(aStrGroups); 
nSubjects=0;
for cg = 1:numel(aStrGroups)
    strGroup = aStrGroups{cg};
    var = aSubject.ATWM1_PSY.Groups.(matlab.lang.makeValidName(aStrGroups{cg}));
    nSubjects=nSubjects+length(var);
end
...
for cg = 1:nGroups
    ...
    for cs = 1:nSubjects(cg)
        iSubject = aSubject.ATWM1_PSY.Groups.(matlab.lang.makeValidName(aStrGroups{cg})){cs};
...
function createBarGraphATWM1_PSY_EXP8(parametersStudy, parametersParadigm, folderDefinition, computedBehavioralData, nSubjects);
...
point2comme_overwrite('D:\Daten\ATWM1\Behavioral_Data\ATWM1_PSY_EXP8_1_group_22_subj_BHD_SPSS.txt') % turns all points into commas
end
