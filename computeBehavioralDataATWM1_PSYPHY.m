function computeBehavioralDataATWM1_PSYPHY ();
% ATWM1
...
handleFunction = str2func(sprintf('selectReprocessingOption%s', iStudy));
[bReprocessAllData, bAbortFunction] = feval(handleFunction, iStudy, parametersStudy);
...
handleFunction = str2func(sprintf('prepareGroupInformation%s', iStudy));
[aSubject, nGroups, nSubjects] = feval(handleFunction, parametersStudy, parametersParadigm, aSubject);
    ...
    for cs = 1:nSubjects(cg)
        ...
        for cco = 1:parametersParadigm.nConditions
            for cr = 1:parametersParadigm.nRunsPerCondition(cco)
                strLogFileName = strcat(iSubject, '-', iStudy, '_', parametersStudy.iPsychophysics, '_', parametersStudy.iExperiment, num2str(parametersStudy.experimentNumber), '_', parametersParadigm.aConditions{cco}, '_', parametersStudy.iRun, num2str(cr), '.log');
                strLogFileName = strcat(iSubject, '-', iStudy, '_', parametersStudy.iPsychophysics, '_', parametersStudy.iExperiment, num2str(parametersStudy.experimentNumber), '_', parametersParadigm.aConditions{cco}, '_', 'Run', num2str(cr), '.log');
                test = 'ANGELIKA_TEST-ATWM1_EXP7_PSY_Nonsalient_Cued_Run1.log'
                ...
                handleFunction = str2func(sprintf('readLogfile%s', iStudy));
                tempTrialData = feval(handleFunction, parametersParadigm, strLogFilePath);
                ...
                % trial data
                handleFunction = str2func(sprintf('addTrialInformation%s', iStudy));
                tempTrialData = feval(handleFunction, parametersParadigm, tempTrialData, strBehavioralDataFile, cr);

                handleFunction = str2func(sprintf('determineTrialAccuracy%s', iStudy));
                tempTrialData = feval(handleFunction, parametersParadigm, tempTrialData);

                handleFunction = str2func(sprintf('calculateTrialReactionTime%s', iStudy));
                tempTrialData = feval(handleFunction, parametersParadigm, tempTrialData);
        ...
        handleFunction = str2func(sprintf('saveBehavioralData%s', iStudy));
        feval(handleFunction, parametersParadigm, trialDataRun, strBehavioralDataFile, strPathBehavioralDataFile);
    end
end
...
for cg = 1:nGroups
    for cs = 1:nSubjects(cg)
        ...
        if exist(strPathPerformanceDataFile{cg, cs}, 'file') && bReprocessAllData == false
            ...
            handleFunction = str2func(sprintf('calculateSubjecPerformanceData%s', iStudy));
            singleSubjectPerformanceData = feval(handleFunction, parametersParadigm, parametersAnalysisBehavioralData, strBehavioralDataFile, strPathBehavioralDataFile);
...
handleFunction = str2func(sprintf('writeComputedBehavioralDataToFile%s', iStudy));
[strSpssBehavioralDataFile, strExcelBehavioralDataFile, successTextFileCreation, successExcelFileCreation] = feval(handleFunction, parametersStudy, parametersParadigm, parametersAnalysisBehavioralData, folderDefinition, aSubject, nGroups, nSubjects, subjectPerformanceData);
...
createBarGraphATWM1_PSYPHY_EXP4(parametersStudy, parametersParadigm, folderDefinition, computedBehavioralData, nSubjects);
...
% => ignore first introduction example in run 1 (0_6_Objects_)
...
if parametersAnalysisBehavioralData.bCreateExcelFileForResults == false
    ...
    % Prepare an excel compatible column indexing system be concatenating
    % letters
...
%ets = strCondition
%etst = aStrConditionForComparison{1}
cct = 0;
for ct = 1:parametersParadigm.nTrialsTotal
    %set = strCondition
    %sets = aStrConditionForComparison{ct}
...
function createBarGraphATWM1_PSYPHY_EXP4(parametersStudy, parametersParadigm, folderDefinition, computedBehavioralData, nSubjects);
...
for cco = 1:(parametersParadigm.nConditionsRetrieval / 2)
    ...
    x = computedBehavioralData.meanConditionRetrievalCowansK(iFirstBar:iSecondBar);
    for cb = 1:numel(x)
